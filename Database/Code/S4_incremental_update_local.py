# -*- coding: utf-8 -*-
"""
Created on Mon Jan 15 15:16:56 2024
...
active_vendor_query = \
"""
SELECT DISTINCT TPO_PO_Vendor_Code FROM mmops.dbo.vendor 
WHERE vendor.Active = 'Y'
"""

vendor_df = pd.read_sql(active_vendor_query,engine)

try:     
    for vendor in vendor_df['TPO_PO_Vendor_Code']:
        drop_query = """DELETE FROM mmops.dbo.fac_report
...
# GET CLOSED MPA
closed_fac_df = fac_df.loc[
    ~fac_df['TPO_PO_Vendor_Code'].isin(vendor_df['TPO_PO_Vendor_Code'])].copy()

fac_done_df = pd.DataFrame()
for vendor in vendor_df['TPO_PO_Vendor_Code']:
    get_fac_data = fac_df.loc[
        (fac_df['TPO_PO_Vendor_Code'] == vendor) & \
        (fac_df['TPO_Created_On'] < seven_months_back)]
    new_fac_data = fac_dataset.loc[
        (fac_dataset['TPO_PO_Vendor_Code'] == vendor) & \
        (fac_dataset['TPO_Created_On'] >= seven_months_back)]
    all_fac_data = pd.concat([get_fac_data, new_fac_data], ignore_index=True)
    fac_done_df = pd.concat([all_fac_data,fac_done_df], ignore_index=True)

fac_done_df = pd.concat([fac_done_df, closed_fac_df], ignore_index=True)

fac_done_df.to_feather('factory_report.feather.zstd', compression = 'zstd')

update_query = \
"""UPDATE mmops.dbo.table_update
SET UPDATED = '{}'
