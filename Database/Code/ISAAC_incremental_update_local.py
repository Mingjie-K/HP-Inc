...
def update_build(mpa_df, cycle_query, tbl, update_query):
    ...
    LOC_FROM_NM FROM mmops.dbo.{} WHERE LOC_FROM_NM
    ...
    try:
        with engine.connect() as conn:
            ...
            print('{} has been updated in Personal Database'.format(tbl))
    except:
...
def update_skus(tbl, i_skus_query, mpa_df):
    ...
    skus_query = """SELECT * FROM mmops.dbo.{}""".format(tbl)
    ...
    for field in fields:
        ...
        if (len(update) != 0):
            for x, y, z in zip(update['LOC_FROM_NM'],
                               ...
                               update['PART_NR']):
                if pd.isnull(y):
                    update_query = """UPDATE mmops.dbo.{} SET {} = NULL
                    WHERE PART_NR = '{}' AND LOC_FROM_NM = '{}'""".format(
                        tbl, field, z, x)
                else:
                    update_query = """UPDATE mmops.dbo.{} SET {} = '{}'
                    WHERE PART_NR = '{}' AND LOC_FROM_NM = '{}'""".format(
                        tbl, field, y, z, x)
                try:
                    with engine.connect() as conn:
...
def update_canon_skus(tbl, i_skus_query, fac_name, prod, fac):
    ...
    skus_query = """SELECT * FROM mmops.dbo.{} WHERE FACTORY = '{}'""".format(tbl, fac_name)
            ...
            for x, y in zip(update[replace_field],
                ...
                try:
                    with engine.connect() as conn:
                        conn.execute(text(update_query))
                        conn.execute(text("COMMIT"))
                        print('{} for SKU {} updated for {}'.format(field, y, fac_name))
                except:
                    print('{} SKU Update has failed for table {}'.format(fac_name, tbl))
        # NEW SKUS
        db_df.loc[db_df['_merge'] == 'right_only',
                  main_field] = db_df[replace_field]
...
# INKJET
# =============================================================================
ink_mpas_query = """SELECT DISTINCT ISAAC_MPA_NAME FROM mmops.dbo.vendor 
...
# LASER
# =============================================================================
laser_mpas_query = """SELECT DISTINCT ISAAC_MPA_NAME FROM mmops.dbo.vendor 
