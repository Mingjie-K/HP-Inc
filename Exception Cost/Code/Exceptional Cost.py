...
PR_excel_files = glob.glob(pr_tracker_dir + '/*' + 'combined*' + excel_details
                           + '.xlsx')
deduct_month = 1
...
# =============================================================================
if x == 'Y':
    Exclude_Cost = pd.read_excel('ExceptionCost_Tool.xlsx', sheet_name='Mapping',
                                 skiprows=1,
                                 usecols='G')
    ...
    PR_df = PR_df.loc[PR_df['IC bill/Not Required/Reclass'] != 'IC bill'].copy()
    # Remove status that are cancelled
    PR_df = PR_df.loc[~PR_df['Status'].isin(['Cancelled','CANCELLED'])].copy()
    ...
    def filter_df(df):
        ...
        if (current_month.month == 10):
            ...
            current_FY = month_df.loc[month_df['FY'].isin(current_FY)]['Quarter'].unique()
            ...
            os.chdir(pr_tracker_dir)
        elif (current_month.month >=2 and current_month.month <=5):
            ...
            os.chdir(pr_tracker_dir)
        elif (current_month.month >=6 and current_month.month <=9):
        ...
        return df, current_FY, current_month, hist_df

    PR_df, current_FY, current_month, hist_df = filter_df(PR_df)
    ...
    Budget_df = Budget_df.loc[Budget_df['Fiscal Quarter'].isin(current_FY)]
